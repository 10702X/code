...
void AWP_solo()
{
  ...
  chassis.drive_distance(-26,41,12,12,1,300,700);
  chassis.drive_distance(-15, 41, 5, 6, 1, 300, 500);
  tilt.set(true);
  wait(0.03,sec);
  //get the last ring
...
void middlegoal_rightside()
{
  ...
  //put ring on goal
    ...
    dunk();
    intak.stop();
    chassis.drive_distance(30, 240, 10, 6, 1, 300, 1500);
  /*//go clear corner
    chassis.drive_max_voltage = 10;
    chassis.turn_max_voltage = 7;
    chassis.drive_timeout = 900;
...
void four_rings()
{
  ...
  dunk();
  frontBrake();
  chassis.drive_distance(21,123, 6, 6, 1, 300, 1500);
  }
}// end of four_rings*******************/
...
{
  loading = 1;
  if (ringCatcher.position(degrees)>20)
  { 
...
void skills()
{
 ...
 // ringColorSensor.objectDetected(ringDetected);   
  ...
  //get mobile goal
  chassis.drive_distance(17, 0, 12, 6, 1, 300, 900);
  chassis.turn_timeout = 700;
  chassis.turn_to_angle(270);
  chassis.drive_distance(-10, 270, 12, 6, 1, 300, 500);
  chassis.drive_distance(-16.5, 270, 6, 6, 1, 300, 1000);
  tilt.set(true);
  wait(0.3,sec);
  ...
  intak.stop();
  tilt.set(false);
  chassis.drive_distance(13); //move out of the corner
  chassis.turn_timeout = 1000;
  chassis.turn_to_angle(90);
  ...
  chassis.drive_distance(-20, 90, 6, 6, 1, 300, 1200); //og 15
  tilt.set(true);
  wait(0.5,sec);
  ...
  chassis.drive_distance(-15, 221, 6, 6, 1, 300, 650); //og 13
  tilt.set(true);
  wait(0.5,sec);
  ...
  chassis.drive_distance(-11, 270, 6, 6, 1, 300, 1000); //og 9
  intak.stop();  
  tilt.set(true);
...
void usercontrol(void) {
  ...
  while (1) {
    ...
    if(Controller1.ButtonL1.pressing()){  // ring catcher going up
      ...
      //ringCatcher.spin(fwd,20,velocityUnits::pct);
      if (ringCatcherRotation.position(degrees)<120)
      {ringCatcher.spin(fwd,3,voltageUnits::volt);}
      else {ringCatcher.stop();}
  
    }
    else if(Controller1.ButtonL2.pressing()){  //ring catcher going down
      if (ringCatcherRotation.position(degrees)>2)
