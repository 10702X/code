...
void AWP_solo_rightside()
{
  ...
  Lift.spinToPosition(500,degrees,false);
 ...
 // intak.stop();
  ...
  Lift.setBrake(brake);
  Lift.spinToPosition(170,degrees,true);
 ...
 // wait(0.3,sec);
  ...
  chassis.drive_timeout = 500;
//  chassis.drive_distance(-3.0);//-2.0
  chassis.turn_timeout = 500;
  chassis.turn_to_angle(310);//303
  chassis.drive_max_voltage = 12;
  chassis.drive_timeout = 800;
...
//chassis.drive_distance(7);
 ...
 // chassis.drive_distance(12);
  ...
  chassis.drive_distance(-20);//original -32
  intak.stop();
...
//  chassis.drive_distance(-18);
  chassis.drive_distance(-18, 0, 12, 6, 1, 300, 800);
...
void skills()
{
  ...
  //get mobile goal
  chassis.drive_distance(14.5, 0, 12, 6, 1, 300, 900);
  chassis.turn_timeout = 700;
  chassis.turn_to_angle(270);
  chassis.drive_distance(-10, 270, 12, 6, 1, 300, 500);
  chassis.drive_distance(-13, 270, 6, 6, 1, 300, 1000);
  tilt.set(true);
  wait(0.3,sec);
...
void sortingIntake()
{
  ...
  color detectedColor = ringColorSensor.color();
  intak.spin(fwd,100,velocityUnits::pct);
 cout<<"color="<<detectedColor<<" hue="<<ringColorSensor.hue()<<"\n";
  if(current_color_selection==0) //red alliance
  {//if blue ring is detected and in position
    if(RingDistance.objectDistance(mm) < 37 && detectedColor == blue)
    {
